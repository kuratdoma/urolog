...
export function LipusForm({ patientId, initialData, firstSessionDate, onSuccess, onCancel }: LipusFormProps) {
    ...
    const { data: recentLabs, isLoading: labsLoading } = useQuery({
        ...
        queryFn: async () => {
            const labs = await api.clinical.getTetkikler(patientId);
            ...
            return labs.filter(lab => {
                ...
                const labDate = new Date(lab.tarih);
                                    ...
                                    <Card key={lab.id} className="p-3 bg-white shadow-sm border-slate-200 border-l-4 border-l-cyan-500">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <div className="flex justify-between items-center text-[10px] text-slate-400">
                                                ...
                                                <span className="bg-cyan-50 text-cyan-700 px-1 rounded font-bold">{lab.sonuc}</span>
                                            </div>
                                        </div>
                                    </Card>
                                ))}
                    ...
                    <div className="p-4 bg-slate-50/30">
                        <div className="space-y-3">
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-slate-700 ml-1">Son 6 Ayda ED Tedavisi Değişikliği</label>
                                <Input value={formData.ed_tedavisi_6ay} onChange={(e) => handleChange("ed_tedavisi_6ay", e.target.value)} placeholder="Örn: Cialis 5mg rutin" className="bg-white h-9 text-sm" />
                            </div>
                            <PEQuestion 
